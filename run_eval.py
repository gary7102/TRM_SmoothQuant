...
from smoothquant_trm import apply_smoothquant  # SmoothQuant W8A8 PTQ
...
class EvalConfig(pydantic.BaseModel):
    ...
    # SmoothQuant options
    use_smoothquant: bool = False      # whether to enable SmoothQuant W8A8
    sq_alpha: float = 0.7              # SmoothQuant alpha
    sq_max_calib_batches: int = 128    # how many batches for calibration
...
def launch(hydra_config: DictConfig):
    ...
    if config.use_smoothquant:
        if RANK == 0:
            print(f"Applying SmoothQuant: alpha={config.sq_alpha}, "
                  f"max_calib_batches={config.sq_max_calib_batches}")
        # model is already on CUDA inside create_model()
        device = next(eval_state.model.parameters()).device
        ...
        _sq_info = apply_smoothquant(
            ...
            device=device,
            max_calib_batches=config.sq_max_calib_batches,
        )

        if RANK == 0:
            print("SmoothQuant finished.")
